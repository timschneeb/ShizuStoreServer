...
`IconToolDir` (default `tools/icon-render`), `PaparazziTimeout` (default
...
that never enriches. Only the `Testing` environment used by the
integration suite skips the check.
...
the PAT is for). Watch it via `journalctl -u shizuappstore -f` and
`sync_runs` rows. Later passes are cheap: a HEAD-gated fast loop every
